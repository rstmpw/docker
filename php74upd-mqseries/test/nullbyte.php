...
//
// Если на 53 строке расскоментить MQSERIES_MQPMO_NEW_MSG_ID, то сервер будет каждый раз генерить новый id
...
while (!strpos(bin2hex($msgId), '00') && $i<1000) {
		...
		echo 'PUT '.bin2hex($msgId).' LEN '.strlen($msgId).$ln;
		...
		echo 'GET '.bin2hex($msgId).' LEN '.strlen($msgId).$ln;
